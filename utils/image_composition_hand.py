...
class ImageComposition():
    ...
    def __init__(self):
        ...
        self.foreground_size = 0.5 # diện tích foreground so với background
        ...
        for category_dir in self.foregrounds_dir.iterdir():
            ...
            for image_file in category_dir.iterdir():
                if not image_file.is_file():
                    ...
                    continue
                if image_file.suffix != '.png' and image_file.suffix != '.xml':
    ...
    def _generate_images(self):
        ...
        for i in tqdm(range(self.count)):
            ...
            save_filename = f'{i+20000:0{self.zero_padding}}' # e.g. 00000023.jpg
    ...
    def _compose_images(self, foregrounds, background_path):
        ...
        for fg in foregrounds:
            ...
            xml_name = str(fg_path.name).split(".")[0] + ".xml"
            xml_path = list(str(fg_path).split(os.path.sep)[:-1])
            xml_path.append(xml_name)
            xml_path = os.path.sep.join(xml_path)
            ...
            posX = int(random.randint(0, max_x_position))
            posY = int(random.randint(0, max_y_position))
            paste_position = (posX, posY)

            # tính toán vị trí bounding box
            contents = open(xml_path).read() # đọc toàn bộ file
            soup = BeautifulSoup(contents,"xml")
            # bounding_box = soup.find("bndbox")

            # if num_devided == 0:
            #     xmin = int(bounding_box.find("xmin").text) + posX
            #     ymin = int(bounding_box.find("ymin").text) + posY
            #     xmax = int(bounding_box.find("xmax").text) + posX
            #     ymax = int(bounding_box.find("ymax").text) + posY
            # else:
            #     xmin = int(bounding_box.find("xmin").text) // num_devided // 2 + posX
            #     ymin = int(bounding_box.find("ymin").text) // num_devided // 2 + posY
            #     xmax = int(bounding_box.find("xmax").text) // num_devided // 2 + posX
            #     ymax = int(bounding_box.find("ymax").text) // num_devided // 2 + posY
            # bbox_info.append((category,xmin,ymin,xmax,ymax))
            bounding_boxes = soup.find_all("bndbox")
...
if __name__ == "__main__":
    ...
    # python D:\cocosynth\python\image_composition_hand.py --input_dir "D:\cocosynth\datasets\ring_dataset\input" --output_dir "D:\cocosynth\datasets\ring_dataset\output" --count 100 --width 1280 --height 720 